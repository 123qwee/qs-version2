<template>
  <div class="organize_border">
    <div class="query">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-select v-model="organizeType" placeholder="组织类型" clearable @change="handleOrganizeType">
    ...
    <div class="result">
      <happy-scroll ref="vHappyScroll" color="rgba(51,51,51,0.2)" hide-horizontal resize size="8">
        <div>
          <el-row v-for="(item,index) in organizeListData" class='item_content' :key="index" justify="space-around" type="flex">
            <el-col>
          ...
          <div v-if="organizeListData.length === 0" style="text-align: center;">暂无数据</div>
        </div>
      </happy-scroll>
    </div>
     <el-pagination
...
export default {
  ...
  created() {
    ...
    if (
      lscache.get("organizeType") &&
      lscache.get("organizeType").length != 0
    ) {
      this.organizeTypeList = lscache.get("organizeType");
    } else {
      commonAjax.getDict({
        data: { typeIds: constants.DICT_TYPE.ORGANIZETYPE.toString() },
        successFunc: data => {
          //组织类型
          lscache.set(
            "organizeType",
            _.map(data.data["社会组织类型"], item => {
  ...
  methods: {
    ...
    itemClick(item) {
      ...
      VistualEmit.emit("change", {
        keyword: item.socialOrgname.trim(),
        type: this.type,
        id: item.id,
        code: item.gridCode
      });
    ...
    getOrganizeList() {
      ...
      let self = this;
      OrganizeService.getOrganizeList(
        this.region,
        this.type,
        this.organizeType,
        data => {
          self.organizeListData = data.data.list || [];
    ...
    addMarker() {
      ...
      let ids = _.map(this.organizeListData, "gridCode");
      this.$parent.addMarker(ids, "组织");
    }
  }
};
</script>
...
<style lang="sass">
@import "./organize.component.scss";
</style>
